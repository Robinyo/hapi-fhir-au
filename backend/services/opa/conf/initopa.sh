...
# Clean up
curl $CURL_OPTS -XDELETE "http://${OPA_SERVICE_HOST}:${OPA_SERVICE_ADMIN_PORT}/v1/policies/rbac" || true

curl $CURL_OPTS -XDELETE "http://${OPA_SERVICE_HOST}:${OPA_SERVICE_ADMIN_PORT}/v1/data/users" || true

# Policy
policyData=$(cat /organization.rego)
curl $CURL_OPTS -XPUT "http://${OPA_SERVICE_HOST}:${OPA_SERVICE_ADMIN_PORT}/v1/policies/rbac" \
